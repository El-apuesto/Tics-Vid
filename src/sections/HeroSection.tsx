...
const heroVideo = {
  ...
  embedUrl:
    'https://www.youtube.com/embed/NYb64OG_ksg?autoplay=1&loop=1&playlist=NYb64OG_ksg&controls=0&modestbranding=1&rel=0&fs=0&iv_load_policy=3&showinfo=0',
};
        ...
        <div ref={heroRef} className="w-full max-w-6xl">
          <div className="relative w-full aspect-video rounded-3xl overflow-hidden border-4 border-white/10 shadow-2xl bg-black">
            <iframe
              ...
              src={isMuted ? heroVideo.embedUrl : heroVideo.embedUrl.replace('mute=1&', '')}
            ...
            {isMuted && (
              <div className="absolute top-4 right-4 flex flex-col items-center gap-1 z-20">
                <button
                  ...
                  className="w-10 h-10 bg-teal-500/90 hover:bg-teal-500 rounded-lg flex items-center justify-center transition-colors"
                >
                  <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/>
                  </svg>
                </button>
                <span className="text-white text-xs font-medium">tap to listen</span>
              </div>
            )}

            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent" />
            <div className="absolute bottom-4 left-6 text-white max-w-3xl">
